...
router.get("/distinctFactory", async (req, res) => {
  try {
    const result = await query(`select distinct factory from jw_common_table.jwdb_rcll`);
...
router.get("/distinctMccode", async (req, res) => {
  try {
    ...
    const result = await query(
      `select distinct mc_code from jw_common_table.jwdb_rcll where factory = $1`,
      [factory]
    );
    res.status(200).json(result.rows);
  } catch (error) {
