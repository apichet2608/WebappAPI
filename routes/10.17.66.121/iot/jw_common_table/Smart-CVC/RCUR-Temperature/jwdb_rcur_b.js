...
router.get("/distinctFactory", async (req, res) => {
  try {
    const result = await query(
      `select distinct factory from jw_common_table.jwdb_rcur_b`
    );
...
router.get("/distinctMccode", async (req, res) => {
  try {
    ...
    const result = await query(
      `select distinct mc_code from jw_common_table.jwdb_rcur_b where factory = $1`,
      [factory]
    );
    res.status(200).json(result.rows);
  } catch (error) {
