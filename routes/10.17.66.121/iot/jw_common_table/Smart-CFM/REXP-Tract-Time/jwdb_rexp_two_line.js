...
router.get("/distinctFactory", async (req, res) => {
  try {
    const result = await query(
      `select distinct factory from jw_common_table.jwdb_rexp_two_line`
    );
...
router.get("/distinctMccode", async (req, res) => {
  try {
    ...
    const result = await query(
      `select distinct mc_code from jw_common_table.jwdb_rexp_two_line where factory = $1`,
      [factory]
    );
    res.status(200).json(result.rows);
  } catch (error) {
...
router.get("/distinctfirst_lot", async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      distinct first_lot
    from
      jw_common_table.jwdb_rexp_two_line
...
router.get("/dataplot", async (req, res) => {
  try {
    ...
    if (first_lot === "ALL") {
      result = await query(
        `SELECT *
        FROM jw_common_table.jwdb_rexp_two_line
        WHERE factory = $1
        AND mc_code = $2
        AND ptime::timestamp >= (now() - interval '${hours}' hour)
        ORDER BY ptime ASC`,
