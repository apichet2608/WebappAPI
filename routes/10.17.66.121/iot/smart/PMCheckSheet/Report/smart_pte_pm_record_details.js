...
router.get("/report_table", async (req, res) => {
  try {
    const { job_id, work_status } = req.query;
    const conditions = [];
    const queryParams = [];

    if (job_id && job_id !== "") {
      conditions.push(`d.job_id = $${queryParams.length + 1}`);
      queryParams.push(job_id);
    }

    if (work_status && work_status !== "") {
      conditions.push(`d.work_status = $${queryParams.length + 1}`);
    ...
    const queryString = `
      ...
      LEFT JOIN
        smart.smart_pte_pm_record d ON c.job_id = d.job_id 
      ${whereClause}
      ORDER BY
